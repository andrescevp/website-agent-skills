...
description: >
  User-invoked. Builds multiple variants of a component you're working on
  and helps you iterate. Use when the user asks to "build variants of X",
  "try different versions of this component", or invokes component-variant.
license: MIT
compatibility: opencode
allowed-tools: bash, read, write, edit
metadata:
  audience: agents exploring component directions
  workflow: component variant exploration
---
...
Select with a URL search param (`?variant=quiet`), so every variant is a link you can send someone. A floating control sets it; [picker.md](references/picker.md) holds the spec.
...
- `references/picker.md` — the floating variant-picker spec. Load when building the harness.

## Validation checklist

- [ ] One primary axis named; each variant on a distinct position; secondary choices follow from it.
- [ ] Every variant clears `better-interface`'s escalation triggers before entering the picker.
- [ ] Ground read: styling system, tokens, density, voice, rendering context (or neutral defaults stated).
- [ ] Variants hosted on the real page with `?variant=` selectable URLs and real content.
- [ ] Tradeoff table: `Variant | Axis position | Right when | Costs`; no favourite marked.
- [ ] On choice: build the winner properly, delete the rest and the harness.
