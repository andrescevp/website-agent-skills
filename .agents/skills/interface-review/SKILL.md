...
description: >
  User-invoked change review. Reviews your work across multiple categories
  like UI, typography, layout, color, writing and accessibility and gives
  you a detailed analysis of the findings for a branch, pull request,
  commit range, or uncommitted changes. Use when the user invokes
  interface-review or asks to "review this PR", "review my changes".
license: MIT
compatibility: opencode
allowed-tools: bash, read, write, edit, chrome-devtools
metadata:
  audience: agents reviewing changes
  workflow: change-scoped interface review
---
...
The whole invocation is the target, so `/interface-review pr 482` reviews pull request 482. [Scope resolution](references/scope-resolution.md) holds the accepted targets and how each resolves.
...
State the repository facts you found, then offer the routes and wait. [Nothing to review](references/scope-resolution.md#nothing-to-review) holds the facts to gather:
...
Review at most five consumers, ordered by [the rule in Scope resolution](references/scope-resolution.md#expanding-to-consumers), then state how many you did not expand. A sweep with no bound cannot support the coverage it claims, and an unstated cutoff reads as completeness.

### 4. Read the removed lines

Regressions are invisible in the post-change state. Read the `-` side of every hunk against [Removed signals](references/removed-signals.md).
...
Rendered verification is opt-in here (see **Never mutate the working tree**). When the user opts in:

- Run `lighthouse_audit` against the head ref's preview and, when one exists, the base ref.
- Report only `Introduced`/`Regression` metric deltas (e.g. CLS +0.04, accessibility −8) with the exact run recorded; without a run, mark metrics `Not verified`.
...
- `references/scope-resolution.md` — accepted review targets, resolution order and blast-radius expansion.
- `references/removed-signals.md` — what to look for on the `-` side of every hunk.

## Validation checklist

- [ ] Change scope resolved: merge-base checked before the working tree; both counts stated.
- [ ] No `HEAD~1..HEAD` fallback on a clean tree; facts stated, user asked.
- [ ] Blast radius expanded (one hop; two for tokens/primitives); at most five consumers reviewed, cutoff named.
- [ ] `-` side of every hunk read against removed-signals.
- [ ] Every finding classified `Introduced` / `Regression` / `Pre-existing`, confirmed with `git blame` where it matters.
- [ ] Working tree never mutated (`git fetch` only; no checkouts, no stashes).
- [ ] At most three pre-existing findings, outside the cap and outside the verdict.
- [ ] Scope block, coverage table and findings table in the output format; metrics deltas reported when rendered verification was opted in.
