...
description: >
  User-invoked. Renders a component you choose in every state and scenario
  on a temporary page and stress tests it. Use when the user asks to "break
  this component", "stress test the UI", "show all states of X", or invokes
  break-site.
license: MIT
compatibility: opencode
allowed-tools: bash, read, write, edit
metadata:
  audience: agents stress-testing components
  workflow: component stress testing
---
...
It observes rather than judges. A finding here is something that visibly broke on the page, named in the vocabulary of the domain skill that owns the fix. Reviewing code against a standard is `interface-review` and `better-interface`; exploring design alternatives is `component-variant`.

Where `component-variant` insists on the real page, this skill isolates on purpose. You are not judging how the component looks in context. You are checking whether it defends itself when the content is worst-case.
...
[scenarios.md](references/scenarios.md) holds the axes, the values on each and the cue that says whether an axis applies. Walk it against the component and keep only the axes whose cue matches. A text input gets content length and states, never item quantity. A static icon button with a fixed label gets container and environment, never long text.
...
- `references/scenarios.md` — the scenario axes, the values on each and the cue that says whether an axis applies. Load while inferring scenarios.

## Validation checklist

- [ ] One component scoped and restated in one sentence.
- [ ] Scenarios inferred from the component's props, slots and states; dropped axes named with reasons.
- [ ] Harness renders the real imported component in its real environment — no restyling, re-theming or probes.
- [ ] Widths are fixed containers on the page; one load shows them all.
- [ ] One look only; breaks marked on the page under each scenario label.
- [ ] Findings table: `Scenario | Observed | Owner`; `"Everything survived"` is a complete report.
- [ ] Nothing fixed unasked; page left running until the user says done.
