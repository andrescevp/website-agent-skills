...
name: better-ui
description: >
  Use this skill when improving UI in your project across the board. Covers
  things like concentric border radius, optical alignment, contextual icons,
  hit areas and more — surfaces, shadows, motion, icon systems and
  interaction feedback. Use when the user asks to "polish the UI", "improve
  the interface", or when surfaces, icons, or animations look off.
license: MIT
compatibility: opencode
allowed-tools: bash, read, write, edit, chrome-devtools
metadata:
  audience: agents polishing interface details
  workflow: UI polish and motion
---
...
Support polish claims with measurements from the chrome-devtools MCP when a browser is available:

- `performance_start_trace` (reload) → LCP, INP, CLS. INP above 200ms on a high-frequency interaction contradicts the motion-restraint rule; first-frame stutter confirms where `will-change` is needed; layout shift while surfaces or images mount shows a missing outline or transition.
- `lighthouse_audit` (best-practices) → flags `will-change` overuse and `transition: all`, plus DOM-size hints.
- Targets: LCP ≤ 2.5s, INP ≤ 200ms, CLS ≤ 0.1. Report measured values next to the principle they affect; never invent a metric.
...
- `references/animations.md` — CSS, Tailwind and Motion recipes (scale on press, theme-switch transition suppression).
- `references/enter-exit.md` — staggered entrances and subtle exits with exact curves.
- `references/icon-transitions.md` — icon swap recipes: motion-library and dependency-free cross-fade.
- `references/icons.md` — icon sizing, stroke matching and RTL flipping.
- `references/performance.md` — `will-change`, compositing and first-frame stutter.
- `references/surfaces.md` — radius, shadow and outline recipes for surfaces.

## Validation checklist

- [ ] Every value matches the exact numbers in this skill (no approximations).
- [ ] All `references/` links resolve.
- [ ] Browser walk covered hover, focus, active, loading and empty states; motion replayed at 10% speed.
- [ ] Unrun checks marked `Not verified`.
- [ ] Metrics (LCP/INP/CLS) measured and reported when a browser was available.
