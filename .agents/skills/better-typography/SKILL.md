...
description: >
  Use this skill when improving type in your project. Covers type scale,
  spacing, sizing, variable fonts, OpenType features, wrapping, truncation
  and more — everything that makes text render, wrap, and behave well
  across the product. Use when the user asks to "fix the typography",
  "improve text rendering", or when headings, labels, or body copy look off.
license: MIT
compatibility: opencode
allowed-tools: bash, read, write, edit, chrome-devtools
metadata:
  audience: agents improving typography
  workflow: type system and text rendering
---
...
Write every fix in the project's styling system, and use the exact values below rather than familiar-looking equivalents. The [cheat sheet](references/css-cheat-sheet.md) maps each declaration to its Tailwind equivalent.
...
Properties keep working when a non-variable fallback renders. Reserve raw tags for custom axes (`"GRAD" 80`) and niche features (`"ss01" 1`) with no property of their own. Axes and feature tags are listed in [variable-fonts-and-opentype.md](references/variable-fonts-and-opentype.md).
...
Below `18px`, stay at weight `400` or heavier. Weights under `300` are display-only at `28px`+; they disappear at text sizes. Pairing guidance is in [choosing-fonts.md](references/choosing-fonts.md).
...
Solo, default names like `text-sm` are fine when the usage rules are clear. On a team, name sizes by use (`text-body-sm`) so the rules survive other people. Scale construction is in [spacing-and-sizing.md](references/spacing-and-sizing.md).
...
Long lines make it hard for the eye to find the next one. Cap long-form text around 60–75 characters per line. Any unit works, as long as a cap exists and the line length lands in range. See [unit choices and the pixel equivalents](references/wrapping-and-punctuation.md#measure-line-length).
...
Both recipes are in [details-and-accessibility.md](references/details-and-accessibility.md).
...
Typography choices show up in Core Web Vitals:

- `performance_start_trace` (reload) → CLS when webfonts swap (verify `font-display` and preloaded `.woff2`) and LCP when the hero heading is the largest contentful paint.
- `lighthouse_audit` → font-display and layout-shift diagnostics under best practices.
- Targets: CLS ≤ 0.1, LCP ≤ 2.5s. A font that shifts layout or delays LCP is a finding under "Serve the right format".
...
- `references/choosing-fonts.md` — font pairing and family selection.
- `references/css-cheat-sheet.md` — every declaration mapped to its Tailwind equivalent.
- `references/details-and-accessibility.md` — iOS input zoom and underline recipes.
- `references/spacing-and-sizing.md` — type scale construction and sizing rules.
- `references/variable-fonts-and-opentype.md` — axes and feature tags for variable fonts.
- `references/wrapping-and-punctuation.md` — measure units, wrapping and punctuation details.

## Validation checklist

- [ ] `.woff2` preferred; intended weights and styles actually loaded.
- [ ] Type scale with semantic names; heading sizes descend with level.
- [ ] Wrapping declarations used for their stated jobs (`balance`, `pretty`, `break-word`, `nowrap`).
- [ ] Tabular numbers on changing values; truncation keeps full values reachable.
- [ ] `lang`/`dir`/`<bdi>` correct; smart punctuation applied.
- [ ] Metrics (CLS from font swap) reported when a browser was available.
