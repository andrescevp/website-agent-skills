...
description: >
  Use this skill for a single combined review of a screen, flow, or repository
  scope. Combines all of the better-* skills into a single review across
  accessibility, layout, writing, typography, color, and UI polish. Use when
  the user asks for "a review of the interface", "check the UI", or when
  findings span more than one design domain.
license: MIT
compatibility: opencode
allowed-tools: bash, read, write, edit, chrome-devtools
metadata:
  audience: agents reviewing interfaces across design domains
  workflow: cross-domain interface review
---
...
As the umbrella review, gather one supporting metric set per domain when a browser is available:

- accessibility: `lighthouse_audit` (accessibility) — score plus failing checks.
- performance: `performance_start_trace` — LCP, INP, CLS with targets LCP ≤ 2.5s, INP ≤ 200ms, CLS ≤ 0.1.
- best-practices: `lighthouse_audit` flags.

Metrics are evidence for the **Why** cell of a verified finding, never a standalone finding. Report the exact run (URL, device, viewport) under verification.
...
The format lives in [review-format.md](references/review-format.md): scope and coverage, the findings table, verification and the verdict. A review is not finished until its findings are reported there.

## References — load when needed

- `references/review-format.md` — the consolidated output format: scope and coverage block, findings table, verification and verdict. Load before writing the report.

## Validation checklist

- [ ] Scope resolved and stated; surface coverage shown (domains + states inspected).
- [ ] Every available `better-*` owner loaded and applied; missing owners marked `Not reviewed`.
- [ ] Escalation triggers checked before ranking; triggered findings are `HIGH` on sight.
- [ ] Findings table: one row per root cause, `path/to/file:line`, `Before`, `After`, `Why`.
- [ ] At most 15 findings; pre-existing findings in their own section, outside the cap.
- [ ] Lighthouse evidence recorded under verification with the exact run.
- [ ] Verdict: `Block` when any `HIGH` remains, `Approve` otherwise.
