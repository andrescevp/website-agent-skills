...
description: >
  Use this skill when your project must comply with accessibility standards
  and best practices: keyboard support, focus, semantics, ARIA, screen
  readers, labels, hit areas, reduced motion, and zoom. Use when the user
  asks to "make it accessible", "fix a11y", "check keyboard support", or
  when an accessibility audit returns findings.
license: MIT
compatibility: opencode
allowed-tools: bash, read, write, edit, chrome-devtools
metadata:
  audience: agents improving accessibility
...
Never let extended hit areas overlap. Give decorative layers `pointer-events: none`, so a glow never swallows the clicks meant for the control beneath it. Sizes and collision rules are in [hit-areas.md](references/hit-areas.md).
...
Add `autocomplete` with a meaningful `name`, plus the `type` and `inputmode` that summon the right keyboard. Never block paste; users paste passwords and one-time codes. See [forms.md](references/forms.md).
...
Two rules hold regardless of the preference. Autoplaying media needs a visible pause control, and toasts carrying an action or an error stay until dismissed. See [motion-and-zoom.md](references/motion-and-zoom.md).
...
Repeated polite announcements need a stable empty region rendered before its text updates. Dynamically inserted alerts vary in support, so test them on the screen readers you target. See [screen-readers.md](references/screen-readers.md).
...
- `lighthouse_audit` (accessibility, mobile and desktop) → axe-based checks for names, ARIA, contrast, focus and semantics; a failure is a verified finding with the audit's evidence.
- `lighthouse_audit` (best-practices) → color-contrast and tap-target diagnostics.
- `performance_start_trace` → confirm reduced-motion and motion fixes hold CLS ≤ 0.1 and INP ≤ 200ms.
- An audit pass never replaces the keyboard-only and screen-reader walks; report those separately.
...
- `references/focus-and-keyboard.md` — focus indicators, keyboard patterns and roving tabindex.
- `references/forms.md` — labels, autocomplete, validation and error wiring.
- `references/hit-areas.md` — target sizes and collision rules.
- `references/motion-and-zoom.md` — reduced motion and zoom resilience.
- `references/screen-readers.md` — live regions, announcements and SR testing.
- `references/semantics-and-aria.md` — landmarks, button-vs-link and disabled states.

## Validation checklist

- [ ] Native elements first; ARIA removed rather than added where possible.
- [ ] `:focus-visible` styled, verified against adjacent colors and forced-colors.
- [ ] Full keyboard path for every pointer interaction; focus trapped and restored in modals.
- [ ] Hit areas meet WCAG 2.5.8 (24×24 minimum; 44×44 touch where density permits) without overlap.
- [ ] Errors announce a recovery path; dynamic content uses the right live region.
- [ ] `prefers-reduced-motion` honored; no meaning rides on color or motion alone.
- [ ] Automated audit (`lighthouse_audit`) run where a browser exists; manual walks reported separately.
