...
name: better-writing
description: >
  Use this skill when improving product copy in your project and making it
  consistent: voice, tone, buttons, errors, empty states, links, settings
  labels and capitalization. Use when the user asks to "improve the copy",
  "fix the error message", "make the writing consistent", or when UI text
  sounds off.
license: MIT
compatibility: opencode
allowed-tools: bash, read, write, edit
metadata:
  audience: agents improving product copy
  workflow: interface writing
---
...
None. This skill owns copy; rendering details live in `better-typography`, error markup in `better-accessibility`, string-room in `better-layout`.

## Validation checklist

- [ ] Voice recon done: terminology, localization conventions and any style guide noted.
- [ ] One voice; tone flexes with stakes (per the tone table).
- [ ] Direct address; plain words; no fragments assembled around variables.
- [ ] Verb-first button labels; confirmation buttons repeat the consequence.
- [ ] One flow vocabulary; links describe their destination.
- [ ] One capitalization policy; settings describe the ON state.
- [ ] Errors say how to fix, next to where it broke; empty states point forward.
- [ ] Findings in `Severity | Location | Before | After | Why` format; verdict `Block`/`Approve`. Source alone is sufficient verification.
