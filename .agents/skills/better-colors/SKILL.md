...
name: better-colors
description: >
  Use this skill when building a color system or answering anything about
  color in your project. You can generate palettes, use semantic tokens,
  convert between formats, check contrast and more. Use when the user asks
  to "create a palette", "fix the colors", "check contrast", or when tokens,
  themes, or gradients need work.
license: MIT
compatibility: opencode
allowed-tools: bash, read, write, edit, chrome-devtools
metadata:
  audience: agents building color systems
  workflow: color systems and contrast
---
...
For a new system, `oklch()` is the best default, because its numbers behave the way the ramp rules below describe. Everywhere else, a color library produces the same ramp in the project's own notation ([color-formats.md](references/color-formats.md)).
...
That seam is what makes theming possible. Without it, dark mode means auditing every usage to work out which meant "the accent" and which just wanted blue ([token-naming.md](references/token-naming.md)).
...
Both ends stop short of pure black and white, which cannot carry hue at all. Use a color library rather than eyeballing it ([palette-generation.md](references/palette-generation.md)).
...
See [color-usage.md](references/color-usage.md).

## Metrics: Lighthouse

- `lighthouse_audit` (accessibility) → the color-contrast rule reports the exact failing pair and ratio; use it to corroborate measured pairs, then report per "Measure the rendered pair, then report".
- The audit is corroboration, not a replacement: compute the rendered pair (the background actually behind the text) before reporting a value.
...
- `references/color-formats.md` — color notations and conversions.
- `references/color-usage.md` — gradient interpolation spaces and application rules.
- `references/contrast.md` — contrast measurement and thresholds.
- `references/palette-generation.md` — building ramps with perceived-lightness steps.
- `references/palette-structure.md` — ramp roles and the Tailwind `50`–`950` / Radix `1`–`12` mappings.
- `references/token-naming.md` — primitive vs semantic token naming.

## Validation checklist

- [ ] Ramps (not isolated colors); every step has a consuming role.
- [ ] Primitives named by hue, semantics by role; token used only in its role.
- [ ] Hue constant across the ramp; vividness peaks mid-ramp; density at the light end.
- [ ] One color, one meaning; exactly one primary action filled per view.
- [ ] Every pair measured against the rendered background; failing pairs reported, not repainted.
- [ ] Contrast corroborated with `lighthouse_audit` when a browser was available.
